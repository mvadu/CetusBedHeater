...
def update_version(line):
    versioninfo = re.search(
        "\{\s*(?P<version>.*Minor:\s+(?P<minor>\d+).*BuildTime:\s+(?P<buildtime>\S+))", line
    )
    version = versioninfo.group("version")

    line = line.replace(versioninfo.group("minor"), str(
        int(versioninfo.group("minor")) + 1))
    line = line.replace(versioninfo.group("buildtime"),
                        '"{}"'.format(datetime.utcnow().isoformat()))
    versioninfo = re.search(
        "\{\s*(?P<version>.*Minor:\s+(?P<minor>\d+).*BuildTime:\s+(?P<buildtime>\S+))",
        line,
    )
    print("Updated Version {} ==> {}".format(
        version, versioninfo.group("version")))
    return line
...
def update_versionInfo():
    ...
    with open(filename, "r") as f:
        ...
        altered_lines = [
            update_version(line) if re.search(
                "^const VersionInfo", line) else line
            for line in lines
        ]
